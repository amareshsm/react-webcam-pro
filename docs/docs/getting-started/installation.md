...
sidebar_position: 1
title: Installation
description: How to install react-camera-web in your React project.
---
...
Before installing `react-camera-web`, make sure you have:
...
`react-camera-web` requires the following peer dependencies to be installed in your project:
...
```bash
npm install react-camera-web react react-dom styled-components
```

## TypeScript Support

`react-camera-web` is written in TypeScript and ships with built-in type declarations. No additional `@types` packages are needed.
...
We're actively fixing open issues from the original `react-camera-pro` repo. If you encounter a bug or need something fixed urgently, [create an issue](https://github.com/amareshsm/react-camera-web/issues/new) in our repository — it will be prioritized and addressed quickly.
:::
