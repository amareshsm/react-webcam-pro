...
sidebar_position: 2
title: v1.0.0
description: Release notes for react-camera-web v1.0.0 — the first stable release.
---
...
🎉 **The first stable release of `react-camera-web`** — a community-maintained fork of [`react-camera-pro`](https://github.com/purple-technology/react-camera-pro).
...
`react-camera-web` picks up where `react-camera-pro` left off. This release addresses all known critical issues while maintaining **full backward compatibility**.
...
`react-camera-web` supports **React 16.8, 17, 18, and 19**. No special configuration needed — just install and use.
